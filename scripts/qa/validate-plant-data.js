...
errors.forEach(e=>console.error(`ERROR: ${e}`));console.log(`Validated ${records.length} record(s).`);if(!records.length)console.warn('WARNING: no records found in data/published or data/drafts — this validator did not check any catalog data (the live catalog is in Supabase).');process.exitCode=errors.length?1:0;
